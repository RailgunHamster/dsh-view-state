...
//
// The selected session is read in a fixed order (see readSessionId): the public
// `sessions.list.getSnapshot().current` first, then the session controller's own
// persisted selection cell `sessions.selection.getSnapshot().sessionId` when the
// list has blanked `current` — measured live, and the defect 0.2.1 exists for.
//
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function run(ctx, warn) {
			...
			//
			// A service is NOT necessarily published yet when this runs, so it is
			// looked up through this getter rather than captured once. Measured on a
			// live web profile (headless Edge over CDP, diagnostic probe in this very
			// file): `ctx.get("sessions")` is `undefined` inside apply(), still
			// `undefined` on the next microtask, and a real service by ~1 s — the
			// session-controller's client half lands after this plugin. Reading it
			// once therefore disabled the session channel for the whole page life.
			...
			const readSessions = () => safeGet(ctx, "sessions");
			...
			// --- session state, bound as soon as the service exists ---
			//
			// The binding is remembered by the stores it subscribed to (bindSessions),
			// which is what makes re-binding exact and idempotent.
			let hasBound = false;
			let boundList;
			let boundSelection;
			let sessionWatch = noSessionWatch();
			...
			const readFacts = () => {
				...
				return {
					session: readSessionFact(readSessions(), sessionWatch),
			...
			const mirror = () => {
				try {
					// Bind before reading: a service that has just appeared must be
					// visible to this very mirror, not to the next one. A watch that
					// concludes during the bind republishes through the same callback.
					bindSessions(mirror);
			...
			/**
			 * Bind (or re-bind) to the sessions service if it is published.
			 *
			 * Called on every mirror, so a service that arrives after activation is
			 * picked up on the next one — and there is always a next one: the layout
			 * watch mirrors when its seat appears (measured ~4 s) and every layout
			 * commit mirrors after that. The binding also REPLACES the deep-link watch,
			 * which is how a `dsh_session` parameter that was unresolved while no
			 * service existed is re-evaluated against the list the moment the service
			 * appears, instead of being forgotten.
			 *
			 * The binding is remembered by the stores it subscribed to, so a mirror
			 * that finds the same stores twice does nothing; a service that is unloaded
			 * and re-published has new stores and is bound afresh.
			 * @param onResolved - called whenever the deep-link watch reaches a new
			 *   conclusion, so the mirror can publish it. Measured necessity: the list
			 *   can finish loading AFTER the last layout-driven mirror (layout seat at
			 *   ~4 s, list at ~5 s), and with nothing to republish the restored
			 *   selection stayed `null` in both channels even though the store held it.
			 * @returns true when the service was (re)bound on this call.
			 */
			const bindSessions = (onResolved) => {
				const sessions = readSessions();
				if (sessions === void 0) return false;
				const list = isObject(sessions) ? sessions.list : void 0;
				const selection = isObject(sessions) ? sessions.selection : void 0;
				// Guard on the STORES, not on the service object: the stores are what is
				// subscribed to, and a service that is unloaded and re-published has new
				// ones. Keeping the guard here is what makes this a no-op on the
				// overwhelming majority of mirrors, so the deep-link watch is never
				// restarted while it is still working through its retry schedule.
				if (hasBound && list === boundList && selection === boundSelection) return false;
				...
				if (hasBound && list !== boundList) {
					// The list was replaced (or lost): the old watch has nothing left to
					// watch, so it is replaced by a fresh one for the new list.
					sessionWatch.cancel();
					sessionWatch = noSessionWatch();
				}
				hasBound = true;
				boundList = list;
				boundSelection = selection;
				if (sessionWatch.resolved() === void 0) {
					sessionWatch = startSessionWatch(ctx, sessions, wanted.session, warn, onResolved);
				}
				unsubscribeSessions = subscribeSessions(sessions, mirror);
				return true;
			};
			...
			// Session selection: the public list feed and the controller's persisted
			// selection cell both move the selection, so a change in either one (a
			// user click, the app's restore) republishes the session parameter.
			// Subscribed independently of the layout, so a layout-less boot still
			// captures session switches — and, since 0.2.1, on both stores, whenever
			// they appear.
			bindSessions(mirror);
			...
			// The sessions service is published AFTER this plugin activates (measured:
			// `ctx.get("sessions")` is undefined in apply() and a service by ~1 s), and
			// unlike the layout seat it has no registry to subscribe to. Without this
			// retry the binding never happened when the layout seat resolved first —
			// the last mirror ran before the service existed, so the session channel
			// was silently dead for the whole page life. Mirroring on each attempt is
			// what lets `bindSessions` find the service on a later one.
			const sessionServiceWatch = watchSessions(mirror);

			return () => {
				// `sessionWatch` and `unsubscribeSessions` are read through closures, so
				// this tears down whatever is current at unload, not what existed when
				// the list was written.
				for (const off of [() => unsubscribeSessions?.(), () => sessionWatch.cancel(), layoutWatch.cancel, sessionServiceWatch, () => unsubscribeLayout?.()]) {
		...
		/**
		 * Wait for the sessions service to be published, then stop.
		 *
		 * The client half of `@deepseek-ai/dsh-api-session-controller` is applied after
		 * this plugin, and there is no public registry notification for "this service
		 * appeared" (the slot registry has one, a service does not), so this reuses the
		 * same escalating schedule as the layout seat. Every attempt calls `retry()`,
		 * which binds the service when it finally appears and mirrors the result.
		 * Giving up is silent: an absent sessions service is a supported boot (the URL
		 * and localStorage channels keep working), and the caller's already-earned
		 * warning budget is spent by the paths that actually lost something.
		 * @param retry - called on each attempt (and once immediately).
		 * @returns a cancel function that clears any pending timer.
		 */
		function watchSessions(retry) {
			let attempt = 0;
			let timer = null;
			let done = false;
			const clear = () => {
			...
			const probe = () => {
				...
				try {
					retry();
				} catch { /* the mirror is fail-soft on its own; keep the schedule alive */ }
				if (attempt >= LAYOUT_RETRY_DELAYS.length) {
					done = true;
					clear();
					return;
		...
		/**
		 * The inert watch used while the sessions service is not published yet.
		 * `resolved()` reports "unknown", which is exactly true: no source exists to
		 * read, so no parameter may be removed on its account.
		 * @returns a watch whose only job is to be cancellable.
		 */
		function noSessionWatch() {
		 ...
		 * @param warn - the one-shot warning sink.
		 * @param onResolved - called (at most once) when the watch reaches its
		 *   conclusion, so the session fact can be published immediately instead of
		 *   waiting for an unrelated mirror to happen.
		 ...
		 */
		function startSessionWatch(ctx, sessions, target, warn, onResolved = null) {
			...
			/**
			 * Tell the mirror that a new conclusion is available.
			 *
			 * Called for BOTH outcomes, including the out-of-attempts "still unknown"
			 * one: that is what republishes a selection the app restores after our last
			 * other mirror (measured — the layout seat arrives first, the session list
			 * later, and the store's own value was otherwise never published).
			 * @param value - the conclusion, or undefined while still unknown.
			 */
			const announce = (value) => {
				if (typeof onResolved !== "function") return;
				try {
					onResolved(value);
				} catch { /* mirroring must never break the watch */ }
			};
			...
			const evaluate = () => {
				...
				if (attempt >= SESSION_RETRY_DELAYS.length) {
					...
					// parameter alone rather than erasing a possibly-valid id. The
					// announcement is what lets a later list (and a selection restored
					// alongside it) still reach the URL.
					stop(void 0);
					return;
				}
		 ...
		 *
		 * A live string — from `list.current` or, when that is blank, from the
		 * controller's persisted selection cell — is authoritative. A missing value
		 * only means "absent" once the deep-link watch has concluded; while it is
		 * still waiting for a list that may yet contain the id, the fact stays
		 * unknown so the parameter survives.
		 * @param sessions - the sessions service, or undefined.
		 ...
		 *
		 * Two stores can move the selection, so both are followed when present:
		 * `list` is the public feed the app renders from, and `selection` is the
		 * controller's own persisted cell (see readSelectionStoreId). A list-only
		 * subscription missed a selection that only the persisted cell carried; a
		 * selection-only one would miss the list's masked/blanked states.
		 * @param sessions - the sessions service, or undefined.
		 * @param onChange - change callback.
		 * @returns one unsubscribe function for every subscription made (never throws).
		 */
		function subscribeSessions(sessions, onChange) {
			const offs = [];
			if (sessions === void 0) return () => {};
			// One guard per listener: this runs on the app's own notification path,
			// so a throwing mirror must not stop later listeners being notified.
			const listener = () => {
				try {
					onChange();
				} catch { /* mirroring must never break the app's own notification path */ }
			};
			for (const store of [sessions.list, sessions.selection]) {
				try {
					if (isObject(store) === false || typeof store.subscribe !== "function") continue;
					const off = store.subscribe(listener);
					if (typeof off === "function") offs.push(off);
				} catch {
					/* one store that refuses subscriptions just mirrors less often */
				}
			}
			return () => {
		 ...
		 *
		 * `ctx.sessions.list.getSnapshot().current` is the public read face and is
		 * preferred. It is not always populated, though: the list store's field doc
		 * says a transiently absent selection blanks `current` — measured live, with
		 * `localStorage["dsh.sessions.current"]` holding a session id while
		 * `list.getSnapshot().current` was `undefined` — so the controller's own
		 * persisted cell is the fallback. The layout store is never consulted: it
		 * carries no session at all (its snapshot is exactly `{ panelInfo,
		 * layoutInfo }`).
		 * @param sessions - the sessions service, or undefined.
		...
		function readSessionId(sessions) {
			...
			let readable = false;
			if (isObject(sessions.list) && typeof sessions.list.getSnapshot === "function") {
				readable = true;
				const current = readSessionCurrent(sessions);
				if (current !== null) return current;
			}
			const selected = readSelectionStoreId(sessions.selection);
			if (selected !== null) return selected;
			// `selection` is a real, readable store that holds nothing: the selection
			// is known to be absent, exactly like an empty `list.current`.
			if (isSelectionStore(sessions.selection)) return null;
			return readable ? null : void 0;
		}
		...
		/**
		 * Structural probe for the session controller's persisted selection cell.
		 *
		 * The shipped controller creates it as
		 * `createSnapshotStore({}, { persist: { name: "dsh.sessions.current" } })`
		 * and keeps it as the own property `selection`, so the same store the app
		 * restores from and writes through to localStorage is reachable here:
		 *
		 *     const restored = this.selection.getSnapshot();   // { sessionId, subagentAddress }
		 *     this.manager = new SessionManager(remote, restored.sessionId, …);
		 *
		 * It is declared `private readonly` in the type, so this is a *runtime*
		 * reach and never a promise: the property may disappear in a future version,
		 * in which case the fallback simply does not exist and behaviour is exactly
		 * what it was before it was added.
		 * @param candidate - `sessions.selection`.
		 * @returns true for an object that offers the store's read face.
		 */
		function isSelectionStore(candidate) {
			return isObject(candidate) && typeof candidate.getSnapshot === "function";
		}

		/**
		 * Read `selection.getSnapshot().sessionId` off a structurally valid cell.
		 * @param candidate - `sessions.selection`.
		 * @returns the persisted id, or null when absent, empty, of the wrong shape,
		 *   or unreadable.
		 */
		function readSelectionStoreId(candidate) {
			if (!isSelectionStore(candidate)) return null;
			try {
				const value = candidate.getSnapshot()?.sessionId;
				return typeof value === "string" && value !== "" ? value : null;
			} catch {
				return null;
			}
