...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function run(ctx, warn) {
			...
			// --- restore ---
			const known = restoreSession(ctx, sessions, wanted.session, warn);
			...
			const state = { session: known, sidebar: wanted.sidebar, rightbar: wanted.rightbar };
			...
			return () => {
				for (const off of [unsubscribeSessions, unsubscribeLayout]) {
		 ...
		 * @param warn - the one-shot warning sink.
		 * @returns the session value the mirror should publish: the target id, null
		 *   when the id was resolved and there is no selection (covered by the
		 *   "not listed" branch below), or `undefined` while nothing is known yet.
		 */
		function restoreSession(ctx, sessions, target, warn) {
			...
			const evaluate = () => {
				...
				if (typeof setTimeout === "function") setTimeout(evaluate, delay);
			};
